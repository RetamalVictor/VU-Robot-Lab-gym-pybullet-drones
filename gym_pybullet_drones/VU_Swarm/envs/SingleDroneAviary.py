...
class SingleDroneAviary(BaseMultiagentAviary):
    def __init__(
        self,
        ...
        target_list: Tuple[int, int] = [[-3,-3],[3,3],[-3,3],[3,-3]],
    ...
    def resetTarget(self):
        self.target = self.target_list[np.random.randint(0,4)]
        self.prev_d = np.sqrt((self.target[0] - 0)**2 +(self.target[1] - 0)**2 )
    ...
    def _computeObs(self):
        ...
        # obs = np.array(
        #     [
        #         np.concatenate((states[0][0:2], self.target), axis=0)
        #         for i in range(self.NUM_DRONES)
        #     ]
        # )
        obs = np.concatenate((states[0][0:2], self.target), axis=0)
        # obs = self._clipAndNormalizeState(state[0])
        return obs
    ...
    def _computeReward(self):
        ...
        now = np.sqrt((self.target[0] - states[0])**2 +(self.target[1] - states[1])**2 )
        reward = 0.0
        if (now - self.prev_d) < 0:
            reward += 0.00001
        else:
            reward += -0.001
        self.prev_d = now
    ...
    def _computeDone(self):
        ...
        done = (np.linalg.norm(state[0:2] - self.target)**2) < 0.2
        return done
    ...
    def _clipAndNormalizeState(self, state):
        ...
        clipped_pos_target = np.clip(self.target, -MAX_XY, MAX_XY)
        ...
        normalized_pos_target = clipped_pos_target / MAX_XY
