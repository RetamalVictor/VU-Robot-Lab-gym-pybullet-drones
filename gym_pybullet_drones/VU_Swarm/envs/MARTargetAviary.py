...
class MARLtest(BaseMultiagentAviary):
    def __init__(
        self,
    ...
    ################################################################################
    def _preprocessObs(self, distance_array, own_position, target_position):

        # Remove the distance with self from array
        distance_array = distance_array[distance_array != 0]

        # Concatenate with [own_position, distance_array, target_position ]
        states_preprocessed = np.concatenate(
            (own_position, distance_array, target_position)
        )
    ...
    def _computeObs(self):
        ...
        # Isolate target position
        target_position = states[-1][:2]
        ...
        obs = np.array(
            [
                self._preprocessObs(
                    self.distance_matrix[i], states[i][:2], target_position
                )
                for i in range(self.NUM_DRONES)
            ]
        )
    ...
    def _computeReward(self):
        ...
        TODO:
        - Add the reward for desired distance. (Proximal control from paper)
        ...
        # rewards = np.zeros((self.NUM_DRONES-1,1))
        states = np.array(
            [self._getDroneStateVector(i) for i in range(self.NUM_DRONES)]
        )
        self._computeDistanceMatrix(states)
        # distance_sigma = np.array([ - np.abs((self.desired_distance * self.NUM_DRONES - 1) - sum(self.distance_matrix[i])) for i in range(self.NUM_DRONES -1)])
        # distance_to_target = np.array([-np.linalg.norm(states[i][0:2] - states[-1][0:2]) ** 2 for i in range(self.NUM_DRONES-1)])

        distance_desired = np.zeros((self.NUM_DRONES,))
        for i in range(self.NUM_DRONES):
            d = self.distance_matrix[i][self.distance_matrix[i] != 0]
            distance_desired[i] = np.sum(
                -np.abs(self.desired_distance - d[d > self.sensing])
            )
        # distance_to_target = np.array([-np.linalg.norm(states[i][0:2] - self.target) ** 2 for i in range(self.NUM_DRONES)])
        colisions = np.array(
            [
        ...
        rewards = distance_desired + colisions
        return rewards
