...
class NeuralControl:
    ...
    def __init__(self, env, agent, args, logger=None):
        ...
        self.agent = [agent for _ in range(self.num_drones)]
    ...
    def train(self, episodes):
        ...
        for i in range(episodes):
            score_1, score_2 = self.trainingLoop(duration_sec=10)
            ...
            print(
                "score %.2f" % score_2,
                "trailing 10 games avg %.3f" % np.mean(score_history_2[-10:]),
            )
        filename = f"drones-alpha{self.alpha}-beta{self.beta}-{datetime.now()}.png"
        plotLearning(score_history_2, filename, window=100)
    ...
    def trainingLoop(self, duration_sec=None):
        ...
        for i in range(0, int(duration_sec * self.env.SIM_FREQ), self.aggr_phy_steps):
            ...
            for i in range(len(self.agent)):
                self.agent[i].remember(
                    obs[i], action[i], rewards[i], next_obs[i], int(done)
                )
